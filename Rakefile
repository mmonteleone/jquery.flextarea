...
task :build => [:clean] do
  ...
  # copy src
  cp 'jquery.flextarea.js', 'dist/jquery.flextarea.js'
...
task :release => [:build] do
  root = pwd+'/dist'
  zip_archive = pwd+'/dist/jquery.flextarea.zip'
...
task :test => [:build] do  
  begin
    # mac
    sh("open spec/jquery.flextarea.specs.html")
  rescue
    # windows
    sh("start spec/jquery.flextarea.specs.html")
  end
end
